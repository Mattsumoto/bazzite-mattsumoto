...
4. **Packages** → `bazzite-mattsumoto` → **Package settings** → change visibility to **Public**. The ISO build cannot pull a private image.
5. **Actions** → **build-disk.yml** → **Run workflow** → download the ISO from the run's artifacts when it finishes.
